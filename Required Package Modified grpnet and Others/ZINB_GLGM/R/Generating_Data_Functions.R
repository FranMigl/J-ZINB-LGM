...
Generetwork_NB <- function(net, # ground truth
  ...
  Y <- matrix(MASS::rnegbin(n * m,
                            means, # means from park chosen 1.5
                            theta),
              nrow = n,
              ncol = m) 
  E <- matrix(MASS::rnegbin(n*p, 0, 1e8), nrow = n, ncol = p)
...
k_net_datagen <- function(net,
  ...
  grpnetworks <- list()
  for (i in 1:k){
    grpnetworks[[i]] <- Generetwork_NB(net = net100, seed = seed + i,
                                       n, p, theta, means = means[i])
