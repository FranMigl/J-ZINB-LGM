...
Accuracy <- function(df){
  acc_plot <- ggplot(df, aes(x = Lambda, y = Balanced_Accuracy)) +
    ...
    geom_line() + 
    geom_point() +
    labs(title = paste("Balanced Accuracy with Lambda,", comment(df)), x = "Crescent Lambda", y = "Balanced Accuracy", subtitle = paste("Best Balanced Accuracy Lambda = ", round(df$Lambda[which.max(df$Balanced_Accuracy)],3)), 
...
F1_stat <- function(df){
  F1_plot <- ggplot(df, aes(x = Lambda, y = F1)) +
    ...
    geom_line() + 
    geom_point() +
    labs(title = paste("F1 Statistic with Lambda,", comment(df)), x = "Crescent Lambda", y = "F1", subtitle = paste('Best F1 Lambda =',round(df$Lambda[which.max(df$F1)],3)), 
