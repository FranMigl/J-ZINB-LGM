# The first function just run the method over a temporary grid if lambda is not provided and returns 
# the output networks

zinb_LGM_grp =  function(Xlist,
                         lambda = NULL,
                         nlambda = 50,
                         sym = c("AND", "OR"),
                         theta = NULL,
  ...
  if (is.null(lambda)) {
    ...
    rho_max = find_lammax_grp(Xlist)/(length(Xlist)^2)
    rho_min = ratio * rho_max
    tmp_lams = c(exp(seq(log(rho_min),log(rho_max), length = 15)))
    
    tmp_net = zinb_LGM_net_grp(X = mat_indexes$matricione,
                               lambda = tmp_lams, # crescente!
                               sym = sym,
                               theta = theta,
                               thresh = thresh,
    ...
    nOfEdge = unlist(lapply(tmp_net$hat_net, function(x){igraph::gsize(igraph::graph_from_adjacency_matrix(as(x, "CsparseMatrix"), mode = "undirected"))}))
    # consider that we have lambda in increasing order
    min_lam = tmp_lams[max(which(nOfEdge == max(nOfEdge)))]
    max_lam = tmp_lams[max(which(nOfEdge > 1))]
    lambda = c(exp(seq(log(min_lam), log(max_lam), length = nlambda)))
    rm(tmp_net)
    cat(" Searching Lambda Complete \n")
  } else {
  ...
  net = zinb_LGM_net_grp(X = mat_indexes$matricione,
                         lambda = lambda,
                         sym = sym,
                         theta = theta,
                         thresh = thresh,
  ...
  if (length(ind) == 0) { # Se non ci sono colonne selezionate restituisci risultato vuoto
    ...
    for (iter in 1:nlambda) {
      ...
      if (iter == 1){
        ...
        coef_res <- tryCatch({
          fitem = zilgm_negbin2_grp(y = y, x = X[, Active, drop = FALSE], # ind è cambiato con gli active
                                    k, 
                                    nk, inds, group,
                                    lambda = lambda[iter], theta = theta,
                                    bvec0 = betaprev, # ridotto ai soli elementi di bvec non nulli  
                                    w = weights,
                                    penalty.factor = penalty.factor,
                                    offset = offset,
                                    thresh = 1e-6, EM_tol = 1e-5, EM_iter = 3e+2, tol = 1e-6, maxit = 3e+2)
          ...
          fitem <- zilgm_negbin2_grp(y = y, x = X[, Active, drop = FALSE], # ind è cambiato con gli active
                                     ...
                                     nk, inds, group,
                                     lambda = lambda[iter], theta = theta,
                                     bvec0 = betaprev, # betaprev solo dei coefficienti di interesse
                                     w = weights,
                                     penalty.factor = penalty.factor,
                                     offset = offset,
                                     thresh = 1e-6, EM_tol = 1e-5, EM_iter = 3e+2, tol = 1e-6, maxit = 3e+2)
        ...
        Bmat[(Active[-c(1:k)] - k), iter] = coef_res$bvec[-(1:k)]
